...
class DIMVImputation:
    ...
    def __init__(self, complete_case: bool):
        ...
        # If complete_case is True, use complete case covariant
        # Otherwise use dpers
        self.complete_case = complete_case

    def fit(self, X: np.ndarray, initializing: bool = False, n_jobs=None) -> None:
        """
        Imputation class that use conditional expectation to fill the missing
        data position. The covariance matrix would be computed by the DPER algorithm
        When to use initializing : Initialize = True should be set if there is many
        missing pattern in the dataset for example randomly missing
        Args:
            X (np.ndarray): Xtrain, dataset used to computer the covariance matrix
            initializing (bool, optional):
                Defaults to False;
                when  initializing is set as True, the missing position would
                be all initialize by 0 (also the mean of the scaled)
        ...
        if self.complete_case:
            # index of complete observation
            idx = ~np.isnan(X).any(-1)
            X_complete = X[idx, :]

            # Compute covariance using X_complete
    ...
    def cross_validate(
        ...
        print(
            "Start Cross Validation with alphas = {} and {} % of training set".format(
                alphas, train_percent * 100
            )
        )
        assert (
            train_percent <= 1 and train_percent > 0.05
        ), " train_percent must be in range(0.1, 1] "
    ...
    def filter_features(
        ...
        mlargest_filter = find_largest_elements(
            s_missing_fts, s_avai_fts, self.cov_no_zeros[idx, :].copy(), mlargest
        )
    ...
    def transform(
        ...
        if features_corr_threshold is not None:
            assert 0 <= features_corr_threshold <= 1, "Value must be between 0 and 1"
        ...
        for idx in tqdm(missing_ftss):
            ...
            while len(missing_values) > 0:
                ...
                same_missing_fts_mask = (
                    np.sum(~np.isnan(X[:, s_missing_fts]), axis=1) == 0
                )
